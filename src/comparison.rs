...
use crate::{
    ...
    REPORT_SCHEMA_VERSION, Throughput,
};
...
pub struct MatchedBenchmark {
    pub identity: BenchmarkCaseIdentity,
...
impl ReportDocument {
    /// Load a native benchmark report and retain a digest of the exact bytes.
    pub fn load_from_path(path: impl AsRef<Path>) -> Result<Self, ReportError> {
        ...
        let report = parse_native_report(&bytes, Some(path))?;
        let reference = ReportReference::native(&report, &bytes, Some(path));
    ...
    pub fn as_native(&self) -> &BenchmarkReport {
        match self {
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnsupportedSchema {
                ...
                formatter,
                "benchmark report{} uses schema version {found}, but this version supports {supported}",
                path_suffix(path)
            ),
        }
    }
...
impl Error for ReportError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            ...
            Self::InvalidSchemaVersion { .. } | Self::UnsupportedSchema { .. } => None,
        }
    }
}
...
impl BenchmarkReport {
    ...
    pub fn compare(
        ...
        let current_reference = in_memory_reference(self, ComparisonSide::Current)?;
        let baseline_reference = in_memory_reference(baseline, ComparisonSide::Baseline)?;
...
pub fn compare_reports(
    ...
    match (current, baseline) {
        (
            ...
            baseline,
            baseline_reference.clone(),
            options,
        ),
    }
}

fn compare_native_reports(
    ...
    validate_comparison_report(current, ComparisonSide::Current)?;
    validate_comparison_report(baseline, ComparisonSide::Baseline)?;

    let current_suite = current
        .suite
        .as_deref()
        .filter(|suite| !suite.trim().is_empty())
        .ok_or(ComparisonError::MissingSuite {
    ...
    if current_suite != baseline_suite {
        return Err(ComparisonError::SuiteMismatch {
            current: current_suite.to_string(),
            baseline: baseline_suite.to_string(),
        });
    }
    ...
    let pairs = pair_results_one_to_one(&current.results, &baseline.results);
    let mut matched_current = vec![false; current.results.len()];
    let mut matched_baseline = vec![false; baseline.results.len()];
    let mut matched = Vec::with_capacity(pairs.len());

    for (current_result, baseline_result) in pairs {
        let current_index = current
            .results
            .iter()
            .position(|candidate| std::ptr::eq(candidate, current_result))
            .expect("paired current result belongs to current report");
        let baseline_index = baseline
            .results
            .iter()
            .position(|candidate| std::ptr::eq(candidate, baseline_result))
            .expect("paired baseline result belongs to baseline report");
        matched_current[current_index] = true;
        matched_baseline[baseline_index] = true;
        matched.push(matched_benchmark(current_result, baseline_result));
    }
...
fn reject_duplicate_identities(
    ...
    for (index, result) in results.iter().enumerate() {
        if results[..index]
            .iter()
            .any(|previous| result_identity_matches(result, previous))
        {
            return Err(ComparisonError::DuplicateIdentity {
                side,
                identity: BenchmarkCaseIdentity::from_result(result),
            });
        }
    }
    Ok(())
...
fn matched_benchmark(current: &BenchmarkResult, baseline: &BenchmarkResult) -> MatchedBenchmark {
    ...
    MatchedBenchmark {
        identity: BenchmarkCaseIdentity::from_result(current),
...
fn unmatched_benchmark(result: &BenchmarkResult) -> UnmatchedBenchmark {
    UnmatchedBenchmark {
        identity: BenchmarkCaseIdentity::from_result(result),
...
fn comparison_snapshot(result: &BenchmarkResult) -> ComparisonCaseSnapshot {
...
fn compare_metrics(current: &BenchmarkResult, baseline: &BenchmarkResult) -> Vec<MetricComparison> {
    let mut matched_baseline = vec![false; baseline.stats.metrics.len()];
    let mut comparisons = Vec::new();
    for current_metric in &current.stats.metrics {
        ...
        let current_median = finite(current_metric.median);
        let baseline_median = finite(baseline_metric.median);
...
fn in_memory_reference(
    report: &BenchmarkReport,
    side: ComparisonSide,
) -> Result<ReportReference, ComparisonError> {
    validate_comparison_report(report, side)?;
    ...
    Ok(ReportReference::native(report, &bytes, None))
}
